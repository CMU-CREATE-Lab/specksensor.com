...
var requestVerificationEmailResend = function(email, callback) {
   superagent
         ...
         .end(function(err, res) {
                 ...
                 // if not a JSend response, then create a new error
                 return callback(new Error("Unexpected error: ESDR service responded with HTTP " + res.statusCode + " while trying to request a verification email resend to [" + email + "]"));
              }
   );
};
...
var getProductByName = function(productName, callback) {
   productName = (productName || "").trim();
   if (productName.length > 0) {
      superagent
            .get(ESDR_API_ROOT_URL + "/products/" + productName)
            .end(function(err, res) {
                    if (err) {
                       return callback(err);
                    }
                    ...
                    // if not a JSend response, then create a new error
                    return callback(new Error("Unexpected error: ESDR service responded with HTTP " + res.statusCode + " while trying to get product [" + productName + "]"));
                 }
      );
   }
   else {
 ...
 * no authorization is sent with the request.
 *
 * @param {number, string} idOrApiKey The ID or API key of the feed to find
 * @param {string[]} requestedFields Array of field names to retrieve.  Will retrieve all fields if <code>null</code> or empty.
 * @param {function} callback The callback function to receive the error or success data
 */
var findFeedByIdOrApiKey = function(idOrApiKey, requestedFields, callback) {
   var fields = [];
   ...
   var fieldsQueryString = fields.length > 0 ? "?fields=" + fields.join(',') : '';

   superagent
         .get(ESDR_API_ROOT_URL + "/feeds/" + idOrApiKey + fieldsQueryString)
         .end(function(err, res) {
                 if (err) {
...
module.exports.findFeedByIdOrApiKey = findFeedByIdOrApiKey;